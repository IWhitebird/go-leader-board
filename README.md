...
# Leaderboard Service with Consistent Hashing

A scalable leaderboard service with production-ready Docker configuration featuring consistent hashing for distributed load balancing.

## Features

- **Nginx Load Balancer** with consistent hashing based on game_id
- **Horizontally Scalable** leaderboard service (4 replicas by default)
- **PostgreSQL Database** with optimized schema for leaderboard queries
...
4. Access the leaderboard service:
   ```
   http://localhost/api/v1/leaderboard?game_id=1
   ```

## Architecture

The service uses consistent hashing based on `game_id` to distribute requests across multiple leaderboard service instances. This ensures that:

1. Requests for the same game always go to the same service instance
2. Even distribution of load across all instances
3. Minimal redistribution when scaling up or down
...
To change the number of leaderboard service replicas:

1. Update the `LEADERBOARD_REPLICAS` value in your `.env` file
2. Restart the services:
   ```
   docker-compose up -d --scale leaderboard=${LEADERBOARD_REPLICAS}
   ```
...
3. Enable TLS/SSL for the Nginx load balancer
